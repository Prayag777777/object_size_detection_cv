...
def index():
    ...
    if request.method == 'POST':
        ...
        if action == 'start':
            ...
            # Debugging: Check if the video stream opens successfully
            if not cap.isOpened():
                logging.error(f"Failed to open video stream: {video_url}")
                return "Failed to open video stream. Please check the IP address.", 400
            else:
                logging.info(f"Video stream opened successfully: {video_url}")
                video_streaming = True
                return redirect(url_for('video_feed'))
        elif action == 'stop':
            if cap and cap.isOpened():
...
def video_feed():
    ...
    global cap, video_streaming
    if not video_streaming or not cap.isOpened():
        logging.error("Video stream not started or failed.")
        return "Failed to open the video stream. Please check the IP address and try again.", 400
    return Response(generate_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')
